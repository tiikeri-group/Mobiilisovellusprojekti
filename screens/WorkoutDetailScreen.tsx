          ...
          return (
            <View style={styles.exerciseBlock}>
              <View style={styles.exerciseHeader}>
                ...
                <View
                  style={[styles.typeBadge, isCardio ? styles.cardioBadge : styles.strengthBadge]}
                >
                ...
                <>
                  ...
                  {sets.map((set, i) => (
                    <View key={set.id ?? i} style={styles.row}>
                      ...
                      <Text style={styles.cell}>{set.weight || "0"}</Text>
                      <Text style={styles.cell}>{set.reps || "0"}</Text>
                    </View>
                  ))}
                </>
              )}
              ...
              {isCardio && sets.length > 0 && (
                <>
                  ...
                  {sets.map((set, i) => {
                    const mins = set.weight || "0";
                    const rawSecs = set.reps || "0";
                    const secs = rawSecs.length === 1 ? `0${rawSecs}` : rawSecs;
