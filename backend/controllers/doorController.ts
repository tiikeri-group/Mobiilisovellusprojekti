...
export const unlockDoor = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const numericDoorId = parseInt(door_id, 10);

    // Check if the door exists
    const result = await pool.query('SELECT * FROM doors WHERE door_id = $1', [door_id]);
    if (result.rowCount === 0) {
      return res.status(404).json({ success: false, message: 'Door not found' });
    }

    // Door exists → unlock logic here
    console.log(`Door ${numericDoorId} unlocked`);
